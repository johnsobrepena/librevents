...
import io.librevents.chain.config.factory.ContractEventDetailsFactoryFactoryBean;
...
import io.librevents.chain.service.health.WebSocketHealthCheckService;
import io.librevents.chain.service.health.strategy.HttpReconnectionStrategy;
import io.librevents.chain.service.health.strategy.WebSocketResubscribeNodeFailureListener;
import io.librevents.chain.service.strategy.PollingBlockSubscriptionStrategy;
import io.librevents.chain.service.strategy.PubSubBlockSubscriptionStrategy;
import io.librevents.chain.settings.BlockStrategy;
...
public class NodeBeanRegistrationStrategy {
    ...
    private String registerBlockchainServiceBean(
        ...
        Class<? extends BlockchainService> blockchainService =
                io.librevents.chain.service.Web3jService.class;
    ...
    private String registerNodeFailureListener(
        ...
        if (isWebSocketUrl(node.getUrl())) {
            final LibreventsWebSocketService webSocketService =
                    (LibreventsWebSocketService) web3jService;
            beanDefinition =
                    BeanDefinitionBuilder.genericBeanDefinition(
    ...
    private Web3jService buildWeb3jService(Node node) {
        ...
        if (isWebSocketUrl(node.getUrl())) {
            ...
            WebSocketService wsService = new LibreventsWebSocketService(client, false);
